---
type: Article
title: https://raw.githubusercontent.com/jpablo/vibe-types/main/plugin/skills/scala3/catalog/T13-null-safety.md
...
- \*\*Java interop / flexible types.\*\* Java-originated reference types are loaded as \_flexible types\_ (`T?`) with bounds `T | Null <: T? <: T`. This lets them be used as either nullable or non-nullable depending on context, aligning Scala's safety guarantees with Java's. Flexible types are non-denotable (compiler-only). Recognized `@NotNull` annotations (e.g., `@org.jetbrains.annotations.NotNull`) suppress nullification.
